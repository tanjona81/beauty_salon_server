...
const updatePreference = () => {
  return async (req, res) => {
    try {
      ...
      await service
        .update(req.params.id, id_customer, id_prefere, designation)
        .then((result) => {
          const responseData = {
            status: true,
...
const deletePreference = () => {
  return async (req, res) => {
    try {
      // await mongoose.connect(uri)
      await service
        .delete_preference(req.params.id)
        .then((result) => {
          const responseData = {
            status: true,
...
const find_and_delete_preference = () => {
  return async (req, res) => {
    try {
      // await mongoose.connect(uri)
      const { id_customer, id_prefere, designation } = req.body;
      await service
        .find_and_delete_preference(id_customer, id_prefere, designation)
        .then((result) => {
          const responseData = {
            status: true,
            message: "Preference deleted successfully",
...
const getAllServicePlusPreference = () => {
  return async (req, res) => {
    try {
      // await mongoose.connect(uri)
      await service
        .all_service_plus_prefere(req.user_id)
        .then((result) => {
          const responseData = {
            status: true,
            message: "List of services prefered",
